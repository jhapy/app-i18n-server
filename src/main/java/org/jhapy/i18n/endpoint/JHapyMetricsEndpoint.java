...
public class JHapyMetricsEndpoint {
  ...
  private Map<String, Number> processMetrics() {
    ...
    Collection<TimeGauge> timeGauges =
        Search.in(this.meterRegistry).name(s -> s.contains("process")).timeGauges();
    timeGauges.forEach(
  ...
  private Map<String, Object> garbageCollectorMetrics() {
    ...
    Collection<Timer> timers =
        Search.in(this.meterRegistry).name(s -> s.contains("jvm.gc.pause")).timers();
    timers.forEach(
        timer -> {
    ...
    Collection<FunctionCounter> functionCounters =
        Search.in(this.meterRegistry)
  ...
  private Map<String, Map<String, Number>> databaseMetrics() {
    ...
    Collection<Timer> timers =
        Search.in(this.meterRegistry).name(s -> s.contains("hikari")).timers();
    timers.forEach(
        timer -> {
          String key =
          ...
          for (ValueAtPercentile percentile : percentiles) {
            resultsDatabase
                .get(key)
                .put(
                    String.valueOf(percentile.percentile()),
                    percentile.value(TimeUnit.MILLISECONDS));
          }
        });

    Collection<Gauge> gauges =
        Search.in(this.meterRegistry).name(s -> s.contains("hikari")).gauges();
    gauges.forEach(
        gauge -> {
          String key =
              ...
              operation -> {
                ...
                Collection<Timer> httpTimersStream =
                    this.meterRegistry
                        ...
                        .timers();
                long count =
                    httpTimersStream.stream().map(Timer::count).reduce(Long::sum).orElse(0L);

                if (count != 0) {
                  double max =
                      httpTimersStream.stream()
  ...
  private Map<String, Map<String, Number>> cacheMetrics() {
    ...
    Collection<FunctionCounter> counters =
        Search.in(this.meterRegistry)
    ...
    Collection<Gauge> gauges =
        Search.in(this.meterRegistry).name(s -> s.contains("cache")).gauges();
    gauges.forEach(
        gauge -> {
  ...
  private Map<String, Map> httpRequestsMetrics() {
    ...
    statusCode.forEach(
        code -> {
          ...
          Collection<Timer> httpTimersStream =
              this.meterRegistry.find("http.server.requests").tag("status", code).timers();

...
public class ElementQueryHandler implements BaseQueryHandler<Element, ElementDTO> {
  ...
  @QueryHandler
  public GetElementByIdResponse getById(GetElementByIdQuery query) {
    return new GetElementByIdResponse(converter.asDTO(repository.getById(query.getId()), null));
  }

  @QueryHandler
  public GetElementByNameResponse getByName(GetElementByNameQuery query) {
    return new GetElementByNameResponse(
        converter.asDTO(repository.getByName(query.getName()).orElseThrow(), null));
  }

  @QueryHandler
  public GetAllElementsResponse getAll(GetAllElementsQuery query) {
    return new GetAllElementsResponse(converter.asDTOList(repository.findAll(), null));
  }

  @QueryHandler
  public FindAnyMatchingElementResponse findAnyMatchingElement(FindAnyMatchingElementQuery query) {
    Page<Element> result =
        BaseQueryHandler.super.findAnyMatching(
            query.getFilter(), query.getShowInactive(), converter.convert(query.getPageable()));
    return new FindAnyMatchingElementResponse(converter.asDTOList(result.getContent(), null));
  }

  @QueryHandler
  public CountAnyMatchingElementResponse countAnyMatchingElement(
      CountAnyMatchingElementQuery query) {
    return new CountAnyMatchingElementResponse(
